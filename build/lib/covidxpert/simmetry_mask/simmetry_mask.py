...
def _get_borders(image: np.ndarray, x: int) -> np.ndarray:
    ...
    cut_image_mask = cv2.threshold(cut_image, np.median(
        cut_image)/2, 255, cv2.THRESH_BINARY)[1]
    flipped_mask = cv2.threshold(flipped, np.median(
        flipped)/2, 255, cv2.THRESH_BINARY)[1]
    sum_reflected = normalize_image(
        cut_image.astype(float) + flipped.astype(float))
    sum_mask = cv2.threshold(sum_reflected, np.median(
        sum_reflected)/2, 255, cv2.THRESH_BINARY)[1]
    ...
    borders = normalize_image(
        cut_image_borders | flipped_borders | sum_borders)
...
def get_borders(image: np.ndarray, x) -> np.ndarray:
    ...
    borders = cv2.threshold(borders, np.median(
        borders), 255, cv2.THRESH_BINARY)[1]
    borders = fill_in_small_artefacts(borders, 50)
