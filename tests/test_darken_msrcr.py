...
from covidxpert.utils import darken, darken_msrcr
...
def test_darken():
    for path in tqdm(glob("tests/test_images/*"), desc="Testing darken msrcr"):
        ...
        darken_msrcr = darken_msrcr(blurred_image)
        ...
        axes[4].imshow(darkened_configured_2, cmap="gray")
        axes[4].set_title("Darkened MSRC")

        [ax.set_axis_off() for ax in axes.ravel()]
        fig.tight_layout()
        path = f"tests/darken_msrcr/{os.path.basename(path)}"
